...
import com.sean.pojo.vo.UsersVO;
import com.sean.service.UserService;
import com.sean.utils.ISeanJSONResult;
...
import com.sean.utils.MD5Utils;
...
//    private FastDFSClient fastDFSClient;
    ...
    public ISeanJSONResult registOrLogin(@RequestBody Users user) throws Exception {
        // 0. 判断用户名和密码不能为空
        if (StringUtils.isBlank(user.getUsername())
                || StringUtils.isBlank(user.getPassword())) {
            return ISeanJSONResult.errorMsg("用户名或密码不能为空...");
        }
        ...
        if (usernameIsExist) {
            ...
            if (userResult == null) {
                return ISeanJSONResult.errorMsg("用户名或密码不正确...");
            }
        } else {
            // 1.2 注册
        ...
        return ISeanJSONResult.ok(userVO);
    }
